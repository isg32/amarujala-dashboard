...
- `maxDuration = 60` is set on the heavier routes/pages (the three `/api/export/*` routes, and the Server-Action pages for bulk reader upload, Close Month, and bulk attendance marking) since their default timeout could be too short for a large org on Vercel's Hobby plan (60s is Hobby's hard ceiling; raise further on Pro if still not enough).
